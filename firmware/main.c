...
static int ultraSound_test(void)
{
	...
	delay_ms(2);
    //MODIFICAR PORQUE SE REPITE SIEMPREEEEEE!!!!
    //MODIFICAR PORQUE SE REPITE SIEMPREEEEEE!!!!
    //MODIFICAR PORQUE SE REPITE SIEMPREEEEEE!!!!
    //MODIFICAR PORQUE SE REPITE SIEMPREEEEEE!!!!
	while(true){
		if(ultrasonido_done_read() == 1){
			int d = ultrasonido_dist_read();
	...
	while(true){
		switch(state){
			...
			case 1: 
				...
				servo_cntrl_ctr_write(1);
				delay_ms(1000);
				d[1] = ultraSound_test();
				state = 2;
				break;
			case 2: 
				servo_cntrl_ctr_write(2);
				delay_ms(1000);
				d[2] = ultraSound_test();
				state = 3;
				break;
...
static void avanzar(void){
	...
	infra = infrarrojo();
		...
		switch (infra){
			 case 0:
			      estado = 0x2;
			      ruedas_cntrl_move_write(estado);
				delay_ms(50);
			      break;
			 case 2:
			      estado = 0x4;
